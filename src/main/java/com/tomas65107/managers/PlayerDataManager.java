...
public class PlayerDataManager {
    ...
    public static final File PLAYER_DATA = new File(clearcheck.CLEARCHECK_DIR, "clearcheck_data.json");
    ...
    public static void load() {
        if (FMLEnvironment.dist.isClient()) throw new RuntimeException("This cant be executed on client!");
        try {
            if (!PLAYER_DATA.exists()) return;
            Type type = new TypeToken<Map<String, Object>>() {}.getType();
            Map<String, Object> combined = gson.fromJson(new FileReader(PLAYER_DATA), type);
            if (combined != null) {
